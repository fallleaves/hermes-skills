...
- **Alert tasks that end as "Agent crash x2 — protocol violation" are usually a DEAD PROVIDER, not a hard fix**: a worker that dies before its first tool call (e.g. `HTTP 401: Insufficient balance` from opencode.ai) exits rc=0 without calling `kanban_complete`/`kanban_block`, so the board only shows the protocol violation. Run `hermes kanban runs <task>` / `hermes kanban log <task>` and read the transcript before concluding anything about the service. Under the multiplexed gateway the dispatcher runs in the DEFAULT profile and reads `kanban.default_assignee` / `kanban.orchestrator_profile` from it, so every task is spawned with THAT profile's credentials — whose provider key answers 401. Both values point at `default` again (2026-09-17) after the default profile's `OPENCODE_GO_API_KEY` was replaced with the funded key the other profiles use — the standing rule is now **keep the DEFAULT profile's provider key funded** (a lapse reproduces this failure for every spawn, and a key swap needs a process restart because the dispatcher reads these values once at boot), and check the assignee profile's provider before concluding a service is unfixable.
